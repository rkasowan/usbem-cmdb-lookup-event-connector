...
WRAPPER_FILE = PROJECT / "servicenow" / "USBEMCMDBFieldEnumerator.script_include.js"
...
class ServiceNow:
    ...
    def background(self, script: str, scope: str = "global") -> str:
        ...
        data = urllib.parse.urlencode({"sysparm_ck": match.group(1), "runscript": "Run script", "sys_scope": scope, "script": script}).encode()
        return opener.open(urllib.request.Request(self.instance + "/sys.scripts.do", data=data, headers={"Content-Type": "application/x-www-form-urlencoded"}), timeout=240).read().decode("utf-8", "replace")


def installer(script_source: str, wrapper_source: str) -> str:
    return """(function () {
var SOURCE = __SOURCE__, NAME = __NAME__, SCOPE = __SCOPE__, ROLE = __ROLE__, SCRIPT = __SCRIPT__, WRAPPER = __WRAPPER__;
...
var wrapperId = upsert('sys_script_include', 'name=USBEMCMDBFieldEnumerator^sys_scope=global', {name: 'USBEMCMDBFieldEnumerator', api_name: 'global.USBEMCMDBFieldEnumerator', active: 'true', access: 'public', caller_access: '1', client_callable: 'false', description: 'Global field-enumeration wrapper restricted at runtime to cmdb_ci and descendant tables.', script: WRAPPER, sys_scope: 'global', sys_package: 'global'});
var privilegeId = upsert('sys_scope_privilege', 'source_scope=' + scopeId + '^target_scope=global^target_name=USBEMCMDBFieldEnumerator^target_type=sys_script_include^operation=execute', {source_scope: scopeId, target_scope: 'global', target_name: 'USBEMCMDBFieldEnumerator', target_type: 'sys_script_include', operation: 'execute', status: 'allowed'});
...
gs.print(JSON.stringify({role: roleId, wrapper: wrapperId, wrapper_privilege: privilegeId, script: scriptId, listener: listenerId, push_instance: pushId}));
})();""".replace("__SOURCE__", json.dumps(SOURCE)).replace("__NAME__", json.dumps(NAME)).replace("__SCOPE__", json.dumps(SCOPE)).replace("__ROLE__", json.dumps(ROLE)).replace("__SCRIPT__", json.dumps(script_source)).replace("__WRAPPER__", json.dumps(wrapper_source))


def main() -> None:
    sn = ServiceNow()
    sn.background(installer(CONNECTOR_FILE.read_text(encoding="utf-8"), WRAPPER_FILE.read_text(encoding="utf-8")))
    result = {
        ...
        "wrapper": sn.get("sys_script_include", "name=USBEMCMDBFieldEnumerator^sys_scope=global", "sys_id,name,api_name,active,access,caller_access,sys_scope", 1),
        "wrapper_privilege": sn.get("sys_scope_privilege", "target_name=USBEMCMDBFieldEnumerator^target_type=sys_script_include^operation=execute", "sys_id,source_scope,target_scope,target_name,target_type,operation,status", 1),
